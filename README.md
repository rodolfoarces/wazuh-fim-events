...
You will need to download the files and place them in a directory in the Wazuh Manager (Master or Worker) wodle directory.

```
curl -L https://raw.githubusercontent.com/rodolfoarces/wazuh-fim-events/refs/heads/dev/src/wazuh-fim-events.py && chmod 700 ./wazuh-fim-events.py
curl -L https://raw.githubusercontent.com/rodolfoarces/wazuh-fim-events/refs/heads/dev/src/example.wazuh-fim-events.conf -o ./wazuh-fim-events.conf
```

The `/var/ossec/wodles/wazuh-fim-events.conf` is to be adjusted acordigly to the Wazuh Server API user.

To execute the FIM helper

```
/var/ossec/framework/python/bin/python3 /var/ossec/wodles/wazuh-fim-events.py
```
...
<wodle name="command">
  ...
  <command>/var/ossec/wodles/wazuh-fim-events.py</command>
...
PUT _ingest/pipeline/filebeat-7.10.2-wazuh-alerts-pipeline
{
    ...
    "processors": [
      ...
      },
      {
		...
		{
			"date_index_name": {
				"if": "ctx?.location == 'syscheck'",
				"field": "timestamp",
				"date_rounding": "d",
				"index_name_prefix": "{{fields.index_prefix}}syscheck-",
